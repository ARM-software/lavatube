...
public:
	...
	// Output rewrites accumulated during the first post-processing pass. Entries retain insertion order and are moved
	// into packet-ordered, per-thread lists for the second pass.
	address_rewrite_accumulator global_output_rewrite_queue GUARDED_BY(sync_mutex);
	std::atomic_uint64_t output_markings_added{ 0 };
