...
class parameter(object):
	...
	def print_save(self, name, owner): # called for each parameter
		...
		if self.funcname == 'vkAllocateMemory' and self.name == 'pAllocateInfo':
			...
			z.do('if (p__external_memory && (virtual_memory_properties.memoryTypes[pAllocateInfo_ORIGINAL->memoryTypeIndex].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT))')
			z.brace_begin()
			...
			z.do('if (posix_memalign((void**)&info->pHostPointer, alignment, size) != 0) ABORT("Failed to allocate external memory");')
