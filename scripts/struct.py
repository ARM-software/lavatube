...
hardcoded_read = [ 'VkDataGraphPipelineConstantARM' ]
hardcoded_write = [ 'VkUpdateBufferInfoARM', 'VkDataGraphPipelineConstantARM' ]
...
def struct_header_print(r, selected = None):
	for v in spec.root.findall('types/type'):
		name = v.attrib.get('name')
		if skip(name, selected): continue
		if name in spec.protected_types:
			print('#ifdef %s' % spec.protected_types[name], file=r)
		accessor = 'const %s* sptr' % name
		print('Json::Value json_%s(const callback_context& cb, %s);' % (name, accessor), file=r)
		if name in spec.protected_types:
			print('#endif // %s' % spec.protected_types[name], file=r)
	print(file=r)
...
def struct_header_write(w, selected = None):
	for v in spec.root.findall('types/type'):
		...
		print('void write_%s(lava_file_writer& writer, %s%s%s);' % (name, modifier, accessor, special), file=w)
		if name in spec.protected_types:
...
def struct_impl_print(r, selected = None):
	for v in spec.root.findall('types/type'):
		name = v.attrib.get('name')
		if skip(name, selected): continue
		if name in spec.protected_types:
		...
		z.read = True
		z.do('Json::Value v;')
		z.do('if (sptr == nullptr) return Json::Value();')
		if v.attrib.get('alias'):
			z.do('return json_%s(cb, reinterpret_cast<const %s*>(sptr));' % (v.attrib.get('alias'), v.attrib.get('alias')))
		else:
...
if __name__ == '__main__':
	...
	p = open('generated/struct_print_auto.h', 'w')
	for i in [r, w, p]:
		print('// This file contains only code auto-generated by %s' % os.path.basename(__file__), file=i)
		print(file=i)
		print('#pragma once', file=i)
		print(file=i)
		print('#include "lavatube.h"', file=i)
		if i == p:
			print('#include "jsoncpp/json/value.h"', file=i)
		print(file=i)
	struct_header_read(r)
	struct_header_write(w)
	struct_header_print(p)
	r.close()
	w.close()
	p.close()
	r = open('generated/struct_read_auto.cpp', 'w')
	w = open('generated/struct_write_auto.cpp', 'w')
	p = open('generated/struct_print_auto.cpp', 'w')
	for i in [r, w, p]:
		print('// This file contains only code auto-generated by %s' % os.path.basename(__file__), file=i)
		print(file=i)
	print('#include "struct_read_auto.h"', file=r)
	print('#include "read.h"', file=r)
	print('#include "read_auto.h"', file=r)
	print('#include "helpers_read.h"', file=r)
	print('#include "suballocator.h"', file=r)
	print('#include "helpers_read.h"', file=r)
	print('#include "struct_write_auto.h"', file=w)
	print('#include "write.h"', file=w)
	print('#include "write_auto.h"', file=w)
	print('#include "helpers_write.h"', file=w)
	print('#include "struct_print_auto.h"', file=p)
	print('#include "read_auto.h"', file=p)
	print('#include "helpers_read.h"', file=p)
	for i in [r, w, p]:
		print(file=i)
		print('#pragma GCC diagnostic ignored "-Wunused-variable"', file=i)
		print('#pragma GCC diagnostic ignored "-Wunused-function"', file=i)
		print('#if (__clang_major__ > 12) || (!defined(__llvm__) && defined(__GNUC__))', file=i)
		print('#pragma GCC diagnostic ignored "-Wunused-but-set-variable"', file=i)
		print('#endif', file=i)
		print('#ifndef __clang__', file=i)
		print('#pragma GCC diagnostic ignored "-Warray-bounds"', file=i)
		print('#pragma GCC diagnostic ignored "-Wstringop-overflow"', file=i)
		print('#endif', file=i)
		print(file=i)
	struct_impl_read(r)
	struct_impl_write(w)
