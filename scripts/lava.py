...
out(targets_main, '#include <stdlib.h>')
out(targets_main, '#include "tracetooltests/tostring.h"')
...
out(targets_read, '#include "helpers_read.h"')
out(targets_write, '#include "helpers_write.h"')
out(targets_read, '#include "struct_read_auto.h"')
out(targets_read, '#include "struct_print_auto.h"')
...
out(targets_write_headers, 'void write_extension(lava_file_writer& writer, VkBaseOutStructure* sptr);')
out(targets_read_headers, 'void read_extension(lava_file_reader& reader, VkBaseOutStructure** sptr);')
out(targets_read_headers, 'Json::Value json_extension(const callback_context& cb, const VkBaseOutStructure* sptr);')
out(targets_write_headers + targets_read_headers)
...
out(targets_read, 'void read_extension(lava_file_reader& reader, VkBaseOutStructure** sptr)')
out(targets_read, '{')
...
out(targets_read, 'Json::Value json_extension(const callback_context& cb, const VkBaseOutStructure* sptr)')
out(targets_read, '{')
out(targets_read, '\tif (!sptr) return Json::Value();')
out(targets_read, '\tJson::Value chain(Json::arrayValue);')
out(targets_read, '\twhile (sptr)')
out(targets_read, '\t{')
out(targets_read, '\t\tif (sptr->sType == VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO || sptr->sType == VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO)')
out(targets_read, '\t\t{')
out(targets_read, '\t\t\tsptr = sptr->pNext;')
out(targets_read, '\t\t\tcontinue;')
out(targets_read, '\t\t}')
out(targets_read, '\t\tJson::Value item;')
out(targets_read, '\t\tswitch ((uint32_t)sptr->sType)')
out(targets_read, '\t\t{')
out(targets_read, '\t\t\tdefault:')
out(targets_read, '\t\t\t\titem["sType"] = (Json::UInt64)sptr->sType;')
out(targets_read, '\t\t\t\titem["TODO"] = "pNext serialization not implemented for this sType";')
out(targets_read, '\t\t\t\tbreak;')
for v in spec.extension_structs:
	if v in struct.skiplist: continue
	if v in spec.protected_types:
	...
	out(targets_read, '\t\t\tcase %s:' % spec.type2sType[v])
	out(targets_read, '\t\t\t{')
	out(targets_read, '\t\t\t\tconst %s* typed = reinterpret_cast<const %s*>(sptr);' % (v, v))
	out(targets_read, '\t\t\t\t%s shallow = *typed;' % v)
	out(targets_read, '\t\t\t\tshallow.pNext = nullptr;')
	out(targets_read, '\t\t\t\titem = json_%s(cb, &shallow);' % v)
	out(targets_read, '\t\t\t\titem["type"] = "%s";' % v)
	out(targets_read, '\t\t\t\titem["sType"] = (Json::UInt64)sptr->sType;')
	out(targets_read, '\t\t\t\tbreak;')
	out(targets_read, '\t\t\t}')
	if v in spec.protected_types:
...
out(targets_read, '\t\tsptr = sptr->pNext;')
out(targets_read, '\t}')
out(targets_read, '\treturn chain;')
out(targets_read, '}')
out(targets_read)

out(targets_write)
out(targets_write, 'void write_extension(lava_file_writer& writer, VkBaseOutStructure* sptr)')
out(targets_write, '{')
...
out(targets_read_headers, 'uint32_t update_buffer_packet(uint8_t instrtype, lava_file_reader& reader);')
out(targets_read_headers, 'uint32_t update_tensor_packet(uint8_t instrtype, lava_file_reader& reader);')
out(targets_read_headers, 'void switchboard_packet(uint8_t instrtype, lava_file_reader& reader);')
out(targets_read_headers, 'void image_update(lava_file_reader& reader, uint32_t device_index, uint32_t image_index, uint64_t size, const VkBaseOutStructure* sptr);')
out(targets_read_headers, 'void buffer_update(lava_file_reader& reader, uint32_t device_index, uint32_t buffer_index, uint64_t size, const VkBaseOutStructure* sptr);')
...
out(targets_read)
out(targets_read_headers, 'void clear_callbacks();')
out(targets_read, 'void clear_callbacks()')
out(targets_read, '{')
for f in spec.functions:
	if f in spec.protected_funcs:
		out(targets_read, '#ifdef %s' % (spec.protected_funcs[f]))
